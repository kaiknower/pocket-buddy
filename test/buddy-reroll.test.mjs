...
test('site script uses shared preview helpers instead of hand-built preview text', () => {
  ...
  assert.match(script, /renderBuddyAsciiPreview/)
  assert.match(script, /featured-ascii-buddy/)
  assert.match(script, /species-ascii-buddy/)
...
test('preview styling uses unified result classes instead of legacy stage and avatar classes', () => {
  ...
  assert.match(css, /\.preview-ascii-buddy\b/)
  assert.match(css, /\.featured-ascii-buddy\b/)
  assert.match(css, /\.species-ascii-buddy\b/)
  assert.match(css, /\.hero-stage\s*\{[\s\S]*grid-template-columns:\s*1fr;/)
  assert.match(css, /\.section-head\s*\{[\s\S]*max-width:/)
  assert.match(css, /\.section-head h2\s*\{[\s\S]*overflow-wrap:\s*anywhere;/)
  assert.match(css, /\.species-ascii-buddy[\s\S]*overflow:\s*hidden;/)
  assert.match(css, /\.preview-stat-row\b/)
  assert.match(css, /\.preview-result-badge\b/)
  assert.match(css, /\.featured-dragon\s*\{[^}]*min-height:\s*250px;/)
  assert.match(css, /\.featured-owl\s*\{[^}]*min-height:\s*250px;/)
  assert.match(css, /\.featured-chonk\s*\{[^}]*min-height:\s*250px;/)
